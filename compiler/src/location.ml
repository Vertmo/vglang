let print_position ff pos =
  let open Lexing in
  Format.fprintf ff "%d:%d" pos.pos_lnum pos.pos_cnum

type location = {
  l_start : Lexing.position; [@printer print_position]
  l_end   : Lexing.position  [@printer print_position]
} [@@deriving show]
...
let print_location ff  { l_start = p1; l_end = p2 } =
